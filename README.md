...
- The preview link is then built as a recipient-specific URL such as `preview.html?id=friend-from-bestie-abc123ef`.
...
1. Create a Supabase project and run `supabase-schema.sql` in the SQL Editor.
   - If you already have a `packages` table, run:
     ```sql
     alter table public.packages add column if not exists share_slug text;
     update public.packages set share_slug = id where share_slug is null;
