...
- Once checkout completes, it generates a unique package ID and stores the full package payload in browser storage.
...
Never expose `SUPABASE_SERVICE_ROLE_KEY` or `RAZORPAY_KEY_SECRET` in HTML or browser JavaScript. After deployment, packages are available at `preview.html?id=<package-id>` and can be opened from any device.